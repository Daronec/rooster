...
import 'package:rooster/util/union_state/union_state_list_body.dart' show UnionStateListBody;

/// Полноэкранная загрузка при старте экрана создания задачи (mobile).
class CreateTaskMobileLoading extends StatelessWidget {
  /// Создаёт виджет.
  const CreateTaskMobileLoading({required this.wm, super.key});
...
/// Блок загрузки списков внутри формы (mobile), для [UnionStateListBody].
class CreateTaskMobileListsLoadingPanel extends StatelessWidget {
  /// Создаёт виджет.
  const CreateTaskMobileListsLoadingPanel({super.key});
